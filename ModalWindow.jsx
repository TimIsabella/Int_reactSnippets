...
const ModalWindow = ({ props, setOpen }) => {
    ...
    const modalWindowHeight = (formikState) => {
        if (formikState.values.isAlliance=== 'true') return '63%';
        if (formikState.values.isAlliance === 'false') return '56.8%';
    };

    const modalBannerHeight = (formikState) => {
        if (formikState.values.isAlliance === 'true') return '21.33%';
        if (formikState.values.isAlliance === 'false') return '23.66%';
    };
    ...
    function preparePayload() {
        ...
        payload.isAlliance === 'true' ? (payload.isAlliance = true) : (payload.isAlliance = false);
    ...
    function postRecord(payload) {
        SnippetsService.post(payload).then(postOnSuccess).catch(onError);
    }

    function putRecord(payload) {
        SnippetsService.put(payload).then(putOnSuccess).catch(onError);
    }
        ...
        <React.Fragment>
            ...
            <div
                    ...
                    <div className="modal-middle-container">
                        ...
                        <div className="modal-middle-select-cluster">
                            ...
                            <Form.Select
                                className="modal-input-box"
                                name="isAlliance"
                                defaultValue={payloadModel.isAlliance}
                                value={formikState.values.isAlliance}
                                onChange={formikState.handleChange}>
                                <option value="false">Non-Alliance</option>
                                <option value="true">Alliance</option>
                            </Form.Select>
                        </div>
                    </div>
                </div>
                ...
                <div className="modal-lower">
                    ...
                    {formikState.values.isAlliance === 'true' && (
                        <>
                            <br />
                ...
                <div>
                    {selectionState && formikState.values.isAlliance === 'true' && (
                        <div>
